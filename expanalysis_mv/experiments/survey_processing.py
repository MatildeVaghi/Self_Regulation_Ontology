...
def get_description(survey_name):
    ...
    if survey_name == 'bis_bas_survey':
        ...
        description = """
            Future time perspective Level. Higher means begin more attentive,
            influence by future states.
        """
        mean = False
    ...
    for score,subset in scores.items():
        ...
        if len(subset[0]) == len(score_subset):
            if score=='total':
                ...
                DVs[score] = {'value': score_subset.mean(), 'valence': subset[1]} #check if this is correct
        else:
            print("%s score couldn't be calculated for subject %s" % (score, df.worker_id.unique()[0]))
...
def calc_lon_socsup_pss_post_DV(df):
    ...
    for score,subset in scores.items():
        ...
        description = """
            ...
            refer to the period post covid here for scoring information for scoting ref for social support
